...
if input_choice == "Type List of Numbers (Custom Data)":
    ...
    # UPDATED: Now shows actual sliders instead of just Low/Med/High
    mean_v = st.sidebar.slider("Mean Wind Speed (m/s)", 0.0, 40.0, 15.0)
    std_v = st.sidebar.slider("Turbulence Level (Std Dev)", 0.1, 5.0, 1.8)
    data = np.random.normal(mean_v, std_v, 10)

# --- 2. CALCULATION ENGINE ---
avg_v = np.mean(data)
std_v_calc = np.std(data, ddof=1) if len(data) > 1 else 0.1
...
ax1.set_xlim(-0.05, 1.05); ax1.legend()
...
ax3.text(0.50, 0.4, f"MARKOV:\n{d_mar[0]}", color=d_mar[1], fontsize=12, fontweight='bold', ha='center', bbox=box)
ax3.text(0.85, 0.4, f"VAGUE AI:\n{d_mc[0]}", color=d_mc[1], fontsize=12, fontweight='bold', ha='center', bbox=dict(box, linewidth=3))
